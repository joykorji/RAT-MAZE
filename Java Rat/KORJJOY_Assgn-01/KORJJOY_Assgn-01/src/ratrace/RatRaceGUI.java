...
final class RatFrame extends JFrame implements ActionListener, ChangeListener
{
  ...
  private final void loadRatClasses()
  {
    ...
    while (st.hasMoreTokens())
    {
      ...
      File ratDir = new File(path +"/rats/" ); //    add    + "/ratrace/rats/" to the path variable IF rats are in subpackage
      if (!ratDir.exists())
        continue;
      ...
      for (int ii = 0;  ii < classes.length;  ii++)
      {
        ...
        try
        {
          ...
          Class ratClass = cl.loadClass("rats."+className); //  add prefix "ratrace.rats." + IF rats are in their own package
